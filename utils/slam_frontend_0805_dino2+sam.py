...
class ColorfulSegmentationVisualizer:
    """彩色类别分割可视化器 - 将不同类别用不同颜色显示，支持动态和静态对象"""
    ...
    def __init__(self):
        ...
        self.static_colors = {
            ...
            'traffic light': [0, 255, 0],  # 绿色（交通信号特殊处理）
    ...
    def get_color_for_class(self, class_name: str, is_dynamic: bool = True) -> List[int]:
        ...
        saturation = 0.6 + (self.class_count % 4) * 0.1  # 0.6-0.9
        value = 0.7 + (self.class_count % 3) * 0.15  # 0.7-1.0
    ...
    def create_combined_segmentation_mask(self, image_shape: Tuple[int, int],
                                          ...
                                          static_boxes: np.ndarray, static_labels: List[str], static_scores: np.ndarray,
        ...
        Returns:
            colored_mask: 彩色分割mask [H, W, 3]
            dynamic_color_map: 动态对象颜色映射
            static_color_map: 静态对象颜色映射
            class_mask: 类别ID mask [H, W]
        """
        h, w = image_shape
        ...
        for i, (box, label, score) in enumerate(zip(dynamic_boxes, dynamic_labels, dynamic_scores)):
            ...
            dynamic_color_map[label] = color

            if dynamic_sam_masks is not None and i < len(dynamic_sam_masks) and dynamic_sam_masks[i] is not None:
                mask = dynamic_sam_masks[i].astype(bool)
        ...
        for i, (box, label, score) in enumerate(zip(static_boxes, static_labels, static_scores)):
            ...
            static_color_map[label] = color

            if static_sam_masks is not None and i < len(static_sam_masks) and static_sam_masks[i] is not None:
                mask = static_sam_masks[i].astype(bool)
                colored_mask[mask] = color
                class_mask[mask] = class_id
            else:
                x1, y1, x2, y2 = box.astype(int)
    ...
    def add_combined_legend(self, image: np.ndarray,
        ...
        legend_width = 300
        legend_height = total_classes * 25 + 80  # 为标题和分类留更多空间
        ...
        if dynamic_color_map:
            cv2.putText(legend_image, "Dynamic Objects",
                        ...
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)  # 红色标题
            y_offset += 25
        ...
        if static_color_map:
            ...
            cv2.putText(legend_image, "Static Objects",
                        (legend_x + 10, y_offset),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 100, 0), 2)  # 绿色标题
            y_offset += 25
    ...
    def create_separate_visualizations(self, image_shape: Tuple[int, int],
                                       dynamic_boxes: np.ndarray, dynamic_labels: List[str], dynamic_scores: np.ndarray,
                                       static_boxes: np.ndarray, static_labels: List[str], static_scores: np.ndarray,
                                       dynamic_sam_masks: Optional[List[np.ndarray]] = None,
                                       static_sam_masks: Optional[List[np.ndarray]] = None) -> Tuple[
        ...
        for i, (box, label, score) in enumerate(zip(dynamic_boxes, dynamic_labels, dynamic_scores)):
            ...
            if dynamic_sam_masks is not None and i < len(dynamic_sam_masks) and dynamic_sam_masks[i] is not None:
                mask = dynamic_sam_masks[i].astype(bool)
                dynamic_mask[mask] = color
            else:
                x1, y1, x2, y2 = box.astype(int)
                x1, y1 = max(0, x1), max(0, y1)
                x2, y2 = min(w, x2), min(h, y2)
                dynamic_mask[y1:y2, x1:x2] = color
        ...
        for i, (box, label, score) in enumerate(zip(static_boxes, static_labels, static_scores)):
            ...
            if static_sam_masks is not None and i < len(static_sam_masks) and static_sam_masks[i] is not None:
                mask = static_sam_masks[i].astype(bool)
                static_mask[mask] = color
            else:
                x1, y1, x2, y2 = box.astype(int)
                x1, y1 = max(0, x1), max(0, y1)
                x2, y2 = min(w, x2), min(h, y2)
                static_mask[y1:y2, x1:x2] = color
...
class EnhancedDynamicObjectMasker:
    ...
    def create_colorful_visualization(self, image, frame_idx,
        ...
        try:
            ...
            print(f"🎨 Created colorful visualization for frame {frame_idx}")
            print(f"   - Dynamic classes: {list(dynamic_color_map.keys())}")
            print(f"   - Static classes: {list(static_color_map.keys())}")
            ...
            print(f"❌ Failed to create colorful visualization for frame {frame_idx}: {e}")
            return {}
    ...
    def detect_and_segment(self, image, frame_idx=None):
        ...
        try:
            ...
            print(f"❌ Dynamic object detection failed: {e}")
        ...
        if self.detect_static_objects and static_prompt:
            try:
                ...
                print(f"🏗️  Detected {len(static_boxes)} static objects")
            except Exception as e:
                print(f"❌ Static object detection failed: {e}")
        ...
        if self.enable_colorful_vis:
            try:
                ...
                print(f"❌ Colorful visualization failed: {e}")
