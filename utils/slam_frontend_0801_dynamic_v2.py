...
class EnhancedDynamicObjectMasker:
    ...
    def _extend_ground_upward(self, image, initial_ground_mask, start_y):
        ...
        for y in range(start_y - 1, max(int(h * 0.3), 0), -1):  # 不超过图像的30%高度
            row_colors = image[y, :, :]  # 形状: (W, 3)
            color_distances = np.linalg.norm(row_colors - mean_ground_color, axis=1)  # 形状: (W,)

            # 基于颜色相似性和现有mask的连通性
            threshold = np.linalg.norm(color_std) * 2
            similar_pixels = color_distances < threshold  # 形状: (W,)

            # 只保留与下方地面区域连通的像素
            if y < h - 1:
                below_mask = extended_mask[y + 1, :]  # 形状: (W,)

                # 修复：手动实现简单的膨胀操作，避免cv2.dilate的形状问题
                # 对1D数组进行膨胀
                dilated_below = np.zeros_like(below_mask)
                for i in range(len(below_mask)):
                    # 检查当前位置及其邻居
                    start_idx = max(0, i - 1)
                    end_idx = min(len(below_mask), i + 2)
                    if np.any(below_mask[start_idx:end_idx]):
                        dilated_below[i] = 1

                connected_pixels = np.logical_and(similar_pixels, dilated_below > 0)
...
class FrontEnd(mp.Process):
    ...
    def optimize_shadow_gaussians(self, current_window, iterations=50):
        """
        专门优化阴影区域的高斯体，确保它们与周围地面融合良好
        """
        try:
            shadow_optimized = 0
            ...
            for kf_idx in current_window:
                ...
                if not (hasattr(viewpoint, 'dynamic_mask') and hasattr(viewpoint, 'ground_mask')):
                    continue

                # 检查是否有阴影区域
                dynamic_mask = viewpoint.dynamic_mask.cuda()
                ground_mask = viewpoint.ground_mask.cuda()
                shadow_regions = dynamic_mask & ground_mask

                if shadow_regions.sum() == 0:
                    continue
                ...
                for iter_idx in range(iterations):
                    ...
                    # 计算阴影区域的损失
                    if hasattr(viewpoint, 'repaired_image') and viewpoint.repaired_image is not None:
                        target_image = viewpoint.repaired_image.cuda()
                    else:
                        target_image = viewpoint.original_image.cuda()

                    # 只计算阴影区域的损失
                    shadow_mask_3d = shadow_regions.unsqueeze(0).expand(3, -1, -1)

                    if shadow_mask_3d.any():
                        shadow_loss = torch.nn.functional.mse_loss(
                            image[shadow_mask_3d],
                            target_image[shadow_mask_3d]
                        )
            ...
            if shadow_optimized > 0:
                Log(f"🎯 Optimized shadow gaussians for {shadow_optimized} regions")

        except Exception as e:
            Log(f"Warning: Shadow gaussian optimization failed: {e}")
