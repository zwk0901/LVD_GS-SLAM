...
class GroundingDINODetector:
    ...
    def _detect_original(self, image, text_prompt, confidence_threshold):
        ...
        try:
            ...
            print(f"📐 Processed image size: {image_tensor.shape[2]} x {image_tensor.shape[1]}")
            ...
            print(f"  - boxes范围: {boxes.min().item():.3f} ~ {boxes.max().item():.3f}")
            print(f"  - 前几个boxes: {boxes[:3] if len(boxes) > 0 else 'None'}")

            # 🔧 关键修复：直接转换为像素坐标
            # Grounding DINO返回归一化的cxcywh格式
            ...
            for box in boxes_np:
                ...
                # 转换为像素坐标
                w_pixel = w_norm * original_w
                h_pixel = h_norm * original_h
                ...
                print(f"  转换: cxcywh({cx:.3f},{cy:.3f},{w_norm:.3f},{h_norm:.3f}) -> xyxy({x1},{y1},{x2},{y2})")

            boxes_final = np.array(boxes_xyxy)
            ...
            print(f"🎯 Grounding DINO最终结果:")
            for i, (box, score, label) in enumerate(zip(boxes_final, scores_np, labels)):
                x1, y1, x2, y2 = box
                area = (x2 - x1) * (y2 - y1)
                print(f"   {i + 1}. {label}: [{x1}, {y1}, {x2}, {y2}] area={area} conf={score:.3f}")

            return boxes_final, scores_np, labels
...
class EnhancedDynamicObjectMasker:
        ...
        try:
            ...
            if True:
                self.grounding_detector = GroundingDINODetector(
                    model_path='/home/zwk/下载/groundingdino_swinb_cogcoor.pth',
                    config_path='/home/zwk/下载/S3PO-GS-main/GroundingDINO-main/groundingdino/config/GroundingDINO_SwinB_cfg.py',
                    device=device
                )
            else:
                # 否则使用Hugging Face模型
    ...
    def detect_and_segment(self, image, frame_idx=None):
        ...
        if len(dynamic_boxes) > 0:
            ...
            for i, (box, score, label) in enumerate(zip(dynamic_boxes, dynamic_scores, dynamic_labels)):
                ...
                try:
                    ...
                    print(f"    box数组: {box_array}")
                    print(f"    box范围: [{box_array.min():.3f}, {box_array.max():.3f}]")

                    # 判断是否是归一化坐标 (0-1范围)
                    if np.all(box_array >= 0) and np.all(box_array <= 1):
                        print(f"    📍 检测到归一化坐标")
                        # 归一化的cxcywh格式
                        cx, cy, bw, bh = box_array
                        # 转换为像素坐标
                        x_center = cx * w
                        y_center = cy * h
                        box_width = bw * w
                        ...
                        print(f"    转换: cxcywh({cx:.3f},{cy:.3f},{bw:.3f},{bh:.3f}) -> xyxy({x1},{y1},{x2},{y2})")

                    else:
                        print(f"    📍 检测到像素坐标")
                        # 可能已经是像素坐标，检查是否是xyxy格式
                        if len(box_array) == 4:
                            if box_array[2] > box_array[0] and box_array[3] > box_array[1]:
                                # 已经是xyxy格式
                                x1, y1, x2, y2 = box_array.astype(int)
                                print(f"    已是xyxy格式: ({x1},{y1},{x2},{y2})")
                            else:
                                # 可能是cxcywh像素格式
                                cx, cy, bw, bh = box_array
                    ...
                    # 验证边界框有效性
                    if x2 <= x1 or y2 <= y1:
                        print(f"    ❌ 无效边界框: ({x1},{y1},{x2},{y2})")
                        continue

                    box_area = (x2 - x1) * (y2 - y1)
                    print(f"    ✅ 最终坐标: ({x1},{y1},{x2},{y2}) 面积={box_area}")

                    max_confidence = max(max_confidence, score)

                    self._last_detected_objects.append({
                        'label': label,
                        'box': [x1, y1, x2, y2],  # 保存最终的xyxy坐标
                        'score': score,
                        'frame': frame_idx,
                        'type': 'dynamic'
                    })
                    ...
                    if any(keyword in label.lower() for keyword in vehicle_keywords):
                        ...
                        print(f"    🚗 车辆扩展: ({x1},{y1},{x2},{y2}) -> ({x1_exp},{y1_exp},{x2_exp},{y2_exp})")
                        x1, y1, x2, y2 = x1_exp, y1_exp, x2_exp, y2_exp

                    # 🔧 关键：生成mask
                    mask_before = np.sum(grounding_dino_mask)
                    grounding_dino_mask[y1:y2, x1:x2] = 1
                    mask_after = np.sum(grounding_dino_mask)
                    added_pixels = mask_after - mask_before
                    total_mask_pixels += added_pixels

                    print(f"    📊 添加了 {added_pixels} 个mask像素")
            ...
            print(f"\n🎯 动态对象处理完成:")
            print(f"  - 总mask像素: {total_mask_pixels}")
            print(f"  - 覆盖率: {total_mask_pixels / (h * w) * 100:.2f}%")
        ...
        if self.use_sam:
            ...
            if len(dynamic_boxes) > 0:
                try:
                    ...
                    for i, detected_obj in enumerate(self._last_detected_objects):
                        if detected_obj['type'] != 'dynamic':
                            continue

                        try:
                            # 使用已经处理好的xyxy坐标
                            x1, y1, x2, y2 = detected_obj['box']
                            ...
                            if any(keyword in label.lower() for keyword in vehicle_keywords):
                                width = x2 - x1
                                height = y2 - y1
                                expand_w = int(width * 0.3)
                                expand_h = int(height * 0.3)
                    ...
                    if sam_combined_mask.sum() > 0:
                        print(f"🎯 SAM优化前mask像素: {final_mask.sum()}")
                        final_mask = sam_combined_mask
                        print(f"🎯 SAM优化后mask像素: {final_mask.sum()}")
        ...
        # 🔧 添加最终验证
        final_mask_pixels = np.sum(final_mask)
        print(f"\n📊 最终mask验证:")
        print(f"  - 最终mask像素: {final_mask_pixels}")
        print(f"  - 最终覆盖率: {final_mask_pixels / (h * w) * 100:.2f}%")
        print(f"  - Mask唯一值: {np.unique(final_mask)}")

        if final_mask_pixels == 0:
            print("⚠️  警告: 最终mask完全为空!")
            print("   请检查:")
            print("   1. Grounding DINO是否正确检测到对象")
            print("   2. 坐标转换是否正确")
            print("   3. 边界框是否有效")
...
class FrontEnd(mp.Process):
    def __init__(self, config, model, save_dir=None):
        ...
        if self.enable_dynamic_filtering:
            print("🔧 === 第一帧动态过滤检查 ===")
            print(f"filter_initialization: {self.filter_initialization}")
            if not self.filter_initialization:
                print("⚠️ 警告: filter_initialization=False，第一帧将不会过滤动态对象！")
                print("⚠️ 这会导致车辆等动态对象被包含在初始高斯中！")

                # 🔧 强制启用（可选）
                response = input("是否强制启用第一帧过滤？(y/n): ")
                if response.lower() == 'y':
                    self.filter_initialization = True
                    print("✅ 已强制启用第一帧过滤")

            # 检查模型状态
            if hasattr(self, 'dynamic_masker') and self.dynamic_masker:
                if not self.dynamic_masker.initialization_success:
                    print("❌ 动态遮罩器初始化失败！")
                elif not self.dynamic_masker.grounding_detector:
                    print("❌ Grounding DINO 未正确加载！")
                else:
                    print("✅ 动态遮罩器初始化成功")
        if self.enable_dynamic_filtering:
            # 设置保存目录
    ...
    # 2. 替换 add_new_keyframe 方法
    def add_new_keyframe(self, cur_frame_idx, depth=None, opacity=None, init=False):
        """修复版本：确保第一帧动态对象被正确检测和过滤"""
        print(f"\n🔧 === 处理关键帧 {cur_frame_idx} (init={init}) ===")
        ...
        original_valid_pixels = valid_rgb.sum().item()
        print(f"原始valid_rgb像素数: {original_valid_pixels}")

        # ===== 🔧 修复：强制对第一帧应用动态检测 =====
        should_apply_filtering = False

        if self.enable_dynamic_filtering and hasattr(self, 'dynamic_masker') and self.dynamic_masker is not None:
            if init:
                # 对于初始化帧，检查配置
                if self.filter_initialization:
                    should_apply_filtering = True
                    print("✅ 初始化帧：将应用动态对象过滤")
                else:
                    print("⚠️ 初始化帧：跳过动态对象过滤（filter_initialization=False）")
            else:
                # 对于非初始化帧，总是应用
                should_apply_filtering = True
                print("✅ 非初始化帧：将应用动态对象过滤")

            # 检查遮罩器状态
            if should_apply_filtering and not self.dynamic_masker.initialization_success:
                print("❌ 动态遮罩器初始化失败，跳过过滤")
                should_apply_filtering = False
        else:
            print("❌ 动态过滤未启用或遮罩器未初始化")
        ...
        if should_apply_filtering:
            try:
                ...
                # 🔧 降低置信度阈值，提高检测灵敏度
                original_threshold = self.dynamic_masker.prompt_manager.scene_prompts[
                    self.dynamic_masker.prompt_manager.current_scene
                ]["confidence_threshold"]

                # 对第一帧使用更低的阈值
                detection_threshold = 0.1 if init else original_threshold
                print(f"使用检测阈值: {detection_threshold} (原始: {original_threshold})")

                # 临时修改阈值
                self.dynamic_masker.prompt_manager.scene_prompts[
                    self.dynamic_masker.prompt_manager.current_scene
                ]["confidence_threshold"] = detection_threshold

                # 执行检测
                static_mask_np, repaired_image_np, detected_objects, colorful_results = (
                    self.dynamic_masker.get_static_mask_for_gaussian_init(
                ...
                # 详细统计检测结果
                dynamic_objects = [obj for obj in detected_objects if obj.get('type') == 'dynamic']
                static_objects = [obj for obj in detected_objects if obj.get('type') == 'static']

                print(f"检测结果详情:")
                print(f"  - 动态对象: {len(dynamic_objects)} 个")
                for obj in dynamic_objects:
                    print(f"    {obj['label']} (conf: {obj['score']:.3f})")
                print(f"  - 静态对象: {len(static_objects)} 个")
                for obj in static_objects[:3]:  # 只显示前3个
                    print(f"    {obj['label']} (conf: {obj['score']:.3f})")

                # 检查mask质量
                static_pixels = np.sum(static_mask_np)
                total_pixels = static_mask_np.size
                static_ratio = static_pixels / total_pixels

                print(f"  - Mask统计:")
                print(f"    静态像素: {static_pixels} / {total_pixels}")
                print(f"    静态比例: {static_ratio:.3f}")

                # 🔧 如果检测到的动态对象太少，给出警告
                if len(dynamic_objects) == 0:
                    print("⚠️ 警告: 未检测到任何动态对象！")
                    print("可能的原因:")
                    print("  1. 图像中确实没有动态对象")
                    print("  2. 检测阈值太高")
                    print("  3. Grounding DINO 模型问题")
                    print("  4. prompt 不匹配场景内容")

                    # 🔧 尝试用更激进的检测
                    print("🔧 尝试更激进的检测...")
                    self.dynamic_masker.prompt_manager.scene_prompts[
                        self.dynamic_masker.prompt_manager.current_scene
                    ]["confidence_threshold"] = 0.05  # 极低阈值

                    static_mask_np_retry, _, detected_objects_retry, _ = (
                        self.dynamic_masker.get_static_mask_for_gaussian_init(
                            img_np, frame_idx=cur_frame_idx
                        )
                    )

                    dynamic_objects_retry = [obj for obj in detected_objects_retry if obj.get('type') == 'dynamic']
                    print(f"重试结果: 检测到 {len(dynamic_objects_retry)} 个动态对象")

                    if len(dynamic_objects_retry) > 0:
                        static_mask_np = static_mask_np_retry
                        detected_objects = detected_objects_retry
                        print("✅ 使用重试结果")
                    else:
                        print("❌ 重试仍无结果，可能图像中确实没有目标对象")
                ...
                # 🔧 对第一帧使用更大的扩展核，确保动态对象完全被排除
                kernel_size = 20 if init else 7  # 第一帧用更大的核
                ...
                print(f"Mask扩展 (kernel_size={kernel_size}):")
                print(f"  - 原始dynamic: {dynamic_mask.sum().item()} 像素")
                print(f"  - 扩展后dynamic: {expanded_dynamic_mask.sum().item()} 像素")
                print(f"  - 扩展了: {expanded_dynamic_mask.sum().item() - dynamic_mask.sum().item()} 像素")

                # 应用mask到valid_rgb
                valid_rgb_before = valid_rgb.sum().item()
                valid_rgb = valid_rgb & expanded_static_mask[None]
                valid_rgb_after = valid_rgb.sum().item()

                filtered_pixels = valid_rgb_before - valid_rgb_after
                filter_ratio = filtered_pixels / valid_rgb_before if valid_rgb_before > 0 else 0

                print(f"Valid RGB 过滤:")
                print(f"  - 过滤前: {valid_rgb_before} 像素")
                print(f"  - 过滤后: {valid_rgb_after} 像素")
                print(f"  - 被过滤: {filtered_pixels} 像素 ({filter_ratio:.1%})")
                ...
                viewpoint.colorful_results = colorful_results

                # 🔧 质量检查
                if valid_rgb_after < valid_rgb_before * 0.05:  # 如果剩余像素少于5%
                    print("🚨 严重警告: 过滤后剩余像素极少！")
                    print("🚨 这可能导致高斯初始化失败！")
                    print("建议调整检测参数或检查图像内容")
                elif filter_ratio > 0.5:  # 如果过滤超过50%
                    print("⚠️ 警告: 过滤了超过50%的像素")
                    print("请确认这是预期的结果")
        ...
        print(f"最终valid_rgb像素数: {valid_rgb.sum().item()}")
        print("=" * 50)
        ...
        if self.monocular:
            if depth is None:
                ...
                print(f"初始深度统计: Max: {torch.max(initial_depth).item():.3f}, "
                ...
                valid_depth_pixels = (initial_depth > 0).sum().item()
                print(f"有效深度像素: {valid_depth_pixels}/{initial_depth.numel()} "
                      f"({valid_depth_pixels / initial_depth.numel() * 100:.1f}%)")
                ...
                valid_rgb_np = valid_rgb.cpu().numpy()
